...
GREEN = "#176B45"
WHITE = "#FFFFFF"
COPYRIGHT = "Copyright © 2026 @derekdaydoi. All rights reserved."
...
def png_metadata():
    ...
    metadata.add_text("Title", "Rootwork brand identity")
    return metadata
...
master.save(MASTER, optimize=True, pnginfo=png_metadata())

for size in SIZES:
    dst = ROOT / f"icon-{size}.png"
    master.resize((size, size), Image.Resampling.LANCZOS).save(dst, optimize=True, pnginfo=png_metadata())
    print("wrote", dst.name)

symbol = Image.new("RGBA", (1024, 1024), (0, 0, 0, 0))
draw_symbol(ImageDraw.Draw(symbol), GREEN)
symbol.resize((512, 512), Image.Resampling.LANCZOS).save(
    ROOT / "brand" / "rootwork-symbol.png", optimize=True, pnginfo=png_metadata()
)
print("done")
